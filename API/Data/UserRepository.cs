...
namespace API.Data
{
  ...
  private readonly IMapper _mapper;
        ...
        public async Task<PagedList<MemberDto>>
         GetMembersAsync(UserParams userParams)
        {
        ...
        //    .ToListAsync();
            ...
            query = query.Where(u=>u.UserName != 
              userParams.CurrentUsername);
                query = query.Where(u=>u.Gender == 
              userParams.Gender);

              var minDob= DateTime.Today.AddYears(-userParams.MaxAge-1);
              var maxDob= DateTime.Today.AddYears(-userParams.MinAge);

              query= query.Where(u=> u.DateOfBirth >= 
              minDob && u.DateOfBirth <=maxDob);
